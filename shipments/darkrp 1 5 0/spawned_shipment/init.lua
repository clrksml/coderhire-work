...
function ENT:OnTakeDamage(dmg)
	...
	if IsValid(owner) and self:Getbuyable() then return false end
...
function ENT:SpawnItem(activator)
	...
	if !IsValid(owner) and self:Getbuyable() then
		self:Setbuyable(false)
	end
	
	if self:Getbuyable() and IsValid(owner) then
		if IsValid(activator) then
			local amount = (CustomShipments[contents].price / CustomShipments[contents].amount)
			
			if self:Getbuyprice() != 0 then
				amount = self:Getbuyprice()
			end
			
			if activator != owner then
				if activator:canAfford(amount) then
					activator:addMoney(-amount)
					DarkRP.notify(activator, 1, 4, "You bought " .. CustomShipments[contents].name .. " for " .. GAMEMODE.Config.currency .. amount .. ".")
					
					if IsValid(owner) then
						owner:addMoney(amount)
						DarkRP.notify(owner, 1, 4, activator:Name() .. " bought " .. CustomShipments[contents].name .. " for " .. GAMEMODE.Config.currency .. amount .. ".")
					end
				end
			end
		end
